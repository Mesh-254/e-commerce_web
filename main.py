...
def getLoginDetails():
    ...
    with sqlite3.connect("database.db") as conn:
        ...
        if "email" not in session:
            ...
            cur.execute(
                "SELECT count(productId) FROM kart WHERE userId = ?", (userId, ))
    ...
    if 'email' not in session:
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            try:
                ...
                conn.rollback()
                msg = "Error occured when inserting item to kart"
        conn.close()
        return redirect(url_for('root'))
...
def removeFromCart():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        try:
            cur.execute(
                "DELETE FROM kart WHERE userId = ? AND productId = ?", (userId, productId))
            conn.commit()
